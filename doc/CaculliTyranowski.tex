...
\item Si la tête \texttt{*head} de la base de donnée est \texttt{NULL}, alors la tête devient le nouveau noeud. On arrête la fonction d'ajout là.
...
\item On vérifie si le premier élément de la liste correspond à l'\texttt{id} de la personne que l'on souhaite supprimer.
  \item Si oui, on crée un n\oe{}ud temporaire qui stockera la personne suivante dans la liste.
\end{enumerate}
